...
Routing is the action for taking funds from the callers wallet, and converting it into a token, whether that is a [defi position token](./defiPosition), or a [base token](./baseToken).
...
There are 2 options for using this individual call
    1. EOA: Approving the contract for the route
    2. Smart Wallets: 
